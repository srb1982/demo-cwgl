...
局域网离线部署的村务综合管理平台，采用 **React 18 + Ant Design 5 + FastAPI + SQLite + WebSocket** 技术栈，支持 22 套村务台账、零代码菜单/字段配置、文档智能归档、自动预警、三费收缴面板、离线数据大屏与备份恢复。
...
backend/               FastAPI 后端
  app/                 
    ...
    seed.py            数据库初始化：22套台账表 + 元数据 + 菜单 + 预置字段库
...
生成村民、党员、残疾人、低保、三费收缴等 22 套台账演示数据，随后在预警中心点击"立即扫描"生成预警。
