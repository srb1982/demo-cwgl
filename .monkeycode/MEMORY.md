...
- Instructions:
  - 后端集成测试：`cd backend && python3 -m pytest tests/ -v`；依赖 pytest 与 httpx（已全局安装）。测试通过 conftest.py 自动切换到临时隔离 SQLite 库并执行 seed 初始化，不触碰生产 data/village.db；已扩展为覆盖全部路由模块（字段/台账/菜单/用户/三费/预警/系统/档案/认证/概览/备份/定时任务/家庭联动/脱敏局域网设置/启动配置/通用局域网管理）共 369 项断言（coverage run --source=app -m pytest tests/）。新增后端改动后跑一遍该套件可快速回归。
  - 家庭联动模块（app/services/family.py，仅作用于 villager 台账）：家庭键=household_no，户主=householder=='是'，人数=population（联动自动重算覆盖手填，含Excel导入后统一重算），单人户=户号下1人。多人户户主变更户号/降级/删除会被 400 阻断，须先交接（接口 POST /ledger/villager/transfer-householder）；单人户户主可自由删除/迁出。每户有且仅有一名户主：单人户户主并入已有户主的户时自动降级为普通成员（保留目标户主）；新增成员显式设'是'时原户主自动降级。前端 LedgerPage 有户主 Tag、删除引导交接弹窗、编辑表单红色提示。其余 30+ 台账不受影响。
  - 存量数据认知：生产库 t_villager_info 为既有用户数据（120 户，每户仅 1 条成员记录，但 population 字段手填 3-6 模拟户规模，其中 104 户与实际行数不符）。此属"存量不动"范畴，勿按联动规则重算存量；人口数联动仅在新增/编辑/删除/交接/导入发生时对涉及户生效。seed.py 不生成 villager 演示数据（仅建表与系统配置），测试环境 villager 表为空、由测试自建数据。
  - 实时推送：WebSocket 由 app/services/sync.py 提供（socketio.AsyncServer，挂载 /socket.io），台账增删改/交接/导入、字段/菜单/系统配置变更均调用 notify_data_changed(menu_code)，预警变化调 notify_warning_changed；前端 src/socket/index.ts 监听并刷新。验证方法：node 连接 ws 后触发一次台账变更，应收 data_changed 事件。注意：向生产后端做此类验证会在 data/village.db 留下测试记录，用 DELETE 接口即时清理，或改用隔离临时库。
  - 系统管理新增二级菜单「局域网设置」(sys_lan, /system/lan) 与「脱敏设置」(sys_mask, /system/mask)，仅 admin 可见（menu_list 对非 admin 排除 system 子菜单，但 system 空父节点仍可见属既有行为）。局域网设置读写 sys_config 的 lan_enabled/server_port，LAN 接口 GET /api/system/lan 返回本机 IP 列表+端口+开关；run.py 的 boot_config() 在启动时读取该配置决定监听地址（lan_enabled=0 时 127.0.0.1，端口取 server_port），配置变更需重启后端生效（test_boot_config.py 3 例锁定）。脱敏设置通过 app/services/mask.py 配置驱动（sys_config 的 mask_enabled/mask_fields/mask_rules 三 key，JSON 存储），ledger.py 列表/只读详情脱敏均改读该配置，默认值与原硬编码行为一致；auth.py 旧 mask_value/apply_mask 保留向后兼容。sys_config 的 set_config 对不存在的 key 返回 404（勿改 upsert，有测试锁定该行为）；新增配置 key 需在 seed.py init_db defaults 注册，重启后生效。
  - 通用局域网访问管理（智能端口顺延 V2.1）已内嵌于 LanPage（Web 面板+Python 引擎）：`app/services/launcher.py` 模块级单例 engine 实现状态机（IDLE/PORT_SCANNING/BINDING/RUNNING_LOCAL/RUNNING_LAN，TRANSITIONS 表校验非法转移）+ 端口顺延（socket SO_REUSEADDR 探测，`ss -ltnp`/psutil 识别占用进程）+ 健康检查（urllib 请求 health_path）+ 进程生命周期（Popen start_new_session 进程树清理，deque 环形日志）+ 防火墙自适应（firewall-cmd/netsh，工具缺失时优雅降级不抛错）。路由 `app/routers/launcher.py`（prefix=/api/system/launcher，仅 admin），配置存 sys_config 的 launcher_config（JSON，含 start_command 支持 {PORT}/{HOST} 占位）。注意：sys_config 无唯一约束，写配置用先查后增改；engine 在 pytest 中为模块级单例，测试需 autouse reset()。子进程用 Popen(start_new_session=True)+preexec_fn 设置 PR_SET_PDEATHSIG（Linux），控制台进程任何方式死亡时被管服务自动回收，独立生存测试（kill 后端后子进程 0 残留）已验收通过；Windows 分支用 os.name 判断。前端 LanPage 保留原「本系统局域网访问」卡片 + 新增控制台卡片（状态 Tag/动态地址/智能日志轮询/配置表单）。
